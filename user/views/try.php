...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_to_cart'])) {
    ...
    // If item is found, proceed to insert into the cart
    if ($item) {
        // Assuming user_id is stored in session (you might want to change this)
        $user_id = $_SESSION['user_id']; // Replace with actual user ID from session or login
        ...
        // Optionally add the item to the session cart (for cart persistence during the session)
...
<div class="container-fluid mb-5">
    ...
    <!-- Name and Price in the same row, adjust layout on small screens -->
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <!-- Right Side: Size, Temperature, Quantity, and Price -->
                    <div class="col-md-6">
                        <?php 
                        // Check if the category is Coffee or Non-Coffee
                        if (in_array($item['category'], ['Coffee', 'Non-Coffee'])) {
                            // If sizes exist in the database, create buttons for each size
                            if (!empty($item['size'])) {
                            ...
                            // If temperatures exist in the database, create radio buttons for each temperature
                            ...
                            // Hide Size and Temperature Sections if not Coffee/Non-Coffee
                            echo '<script>
                        ...
                        <!-- Price with Green Color (Label and Value together) -->
...
function toggleButton(button) {
    ...
    // Update hidden input values for size or temperature
...
<script>
// Update quantity value when the range slider is changed
document.getElementById('quantity<?php echo $item['id']; ?>').addEventListener('input', function() {
    var quantity = this.value;
    document.getElementById('quantityLabel<?php echo $item['id']; ?>').textContent = quantity;
    document.getElementById('quantityInput<?php echo $item['id']; ?>').value = quantity;
...
// Update size when a size button is clicked
document.querySelectorAll('[name="size"]').forEach(button => {
    button.addEventListener('click', function() {
        ...
        // Set the value of the hidden size input for the current item
        document.getElementById('sizeInput' + itemId).value = size;
    });
});

// Update temperature when a temperature button is clicked
document.querySelectorAll('[name="temperature"]').forEach(button => {
    button.addEventListener('click', function() {
        const temperature = this.value; // Get the selected temperature
        const itemId = this.closest('form').querySelector('input[name="item_id"]').value; // Get the item ID

        // Set the value of the hidden temperature input for the current item
