...
$sql = "SELECT firstname, lastname, email, contact_number, profile_picture FROM client WHERE id = ?";
    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            case 'updateProfile':
                ...
                if (empty($errorMessages)) {
                    if (empty($newPassword)) {
                        ...
                        $stmt = $conn->prepare($sql);
                        $stmt->bind_param("ssssi", $firstName, $lastName, $email, $contact_number, $userId);
                    } else {
                ...
                if (isset($_FILES['profile_picture']) && $_FILES['profile_picture']['error'] === UPLOAD_ERR_OK) {
                    ...
                    if (in_array($fileType, $allowedTypes)) {
                        ...
                        if (move_uploaded_file($_FILES['profile_picture']['tmp_name'], $uploadPath)) {
                            $sql = "UPDATE client SET profile_picture = ? WHERE id = ?";
                            $stmt = $conn->prepare($sql);
            ...
            case 'deleteProfilePicture':
                ...
                $sql = "UPDATE client SET profile_picture = NULL WHERE id = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("i", $userId);
